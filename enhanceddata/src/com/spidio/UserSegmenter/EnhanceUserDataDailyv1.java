...
private static TransportClient client;
	...
	public static boolean generateData(String startdate, String enddate, Client client) throws Exception {
	...
	//	Client client = new TransportClient()
		...
		SearchResponse response1 = client.prepareSearch("userprofilestore")
				...
				.setQuery(QueryBuilders.matchQuery("channel_name","cuberoot")).setSize(100000).execute()
				.actionGet();
		...
		try{
			...
			while (true) {
				...
				for (SearchHit hit : response1.getHits().getHits()) {
					...
					Runnable worker = new WorkerThread(hit,client);
